...
open class BaseBottomAuto(var mirror: Boolean = false) : LinearOpMode() {
    ...
    private fun buildPath() {
        var startingPose = Pose(0.0, 0.0)
        var shootingPose = Pose(94.0, 0.0, Math.toRadians(45.0))
        var endPose = Pose(120.0, 0.0, Math.toRadians(90.0))
        if (mirror) {
